...
func getKubernetesClient() kubernetes.Interface {
	...
	if err != nil {
		log.Fatalf("Error building kubeconfig: KUBE_CONFIG, and KUBE_MASTER were not set correctly!")
	}
	...
	informer := cache.NewSharedIndexInformer(
		...
		&cache.ListWatch{
			ListFunc: func(options meta_v1.ListOptions) (runtime.Object, error) {
				// list all of the pods (core resource) in all namespaces
				return client.CoreV1().Pods("").List(options)
			},
			WatchFunc: func(options meta_v1.ListOptions) (watch.Interface, error) {
				// watch all of the pods (core resource) in the default namespaces
